...
def load_format_dataset(
    test_path: str, model_precision: torch.dtype
) -> list:
...
def compute_reference_molecular_moments(elements: np.ndarray[str], coordinates: np.ndarray[float]) -> tuple[np.ndarray, np.ndarray]:
    ...
    # Generate grid.dat file so that 'GRID_ESP' method can integrate the ESP over a grid surrounding the molecule
    ...
    surface_points = np.reshape(
        surface_points[None] + coordinates[:, None], [-1, 3]
        )

    surface_points = surface_points[
        np.where(np.all(cdist(surface_points, coordinates) >= (CUTOFF - 1e-1), axis=-1)
        )[0]]
    ...
    ref_quadrupole_moments = wfn_variables['MBIS MULTIPOLE MOMENTS QUADRUPOLE'].to_array()
...
def get_multipole_moments(
    ...
    # Generate 40 random numbers, with seeded value for consistency when using values in PredictNetwork.py
    random.seed(42)
    rand_nums = random.sample(range(len(testgraphs)), 40)
    ...
    for i in range(len(testgraphs)):
        ...
        if i in rand_nums:
            ...
            elements = [conv_one_hot_to_str(elements[i], element_types) for i in range(len(elements))]
            ...
            starttime = time.time()
            mol_quad, mol_oct = compute_reference_molecular_moments(elements, coordinates)
        ...
        graph.ndata["molecular_quadrupole"] = torch.tensor(np.array(molecular_quadrupole))
